...
def quoteScrape(url):
    ...
    while True:
        ...
        time.sleep(1)
        # Not really sure what was going on wit the previous selectors.
        # Selenium has a bug where several By.<options> are defaulting to CSS selector so I've been strong armed into using them.
        first_quote = driver.find_element(
            By.XPATH, '/html/body/div/div[2]').text
        next_button = driver.find_element(
            By.CSS_SELECTOR, 'body > div > nav > ul > li > a')
        # My machine needs the second sleep for selenium to work right. Not need everywhere. Just a budget fix for a silly problem
        time.sleep(1)
        # changed the logic to make a little more sense.
        if not next_button:
            return  # changed from break to return as I put it in a function
